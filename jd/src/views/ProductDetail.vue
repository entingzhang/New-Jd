<template>
	<div>
		<top-bar>
			<ul>
				<li :class="{active:navIndex==index}" v-for="(item,index) in nav" @click="navIndex=index;scroll(item.ref)"><span v-text="item.title"></span></li>
			</ul>
		</top-bar>
		<div id="product" ref="product" style="height: 600px;background-color: red;"></div>
		<div id="comment" ref="comment" style="height: 600px;background-color: pink;"></div>
		<div id="detail" ref="detail" style="height: 600px;background-color: deeppink;"></div>
		<div id="recommend" ref="recommend" style="height: 600px;background-color: green;"></div>
	</div>
</template>

<script>
	import TopBar from "@/components/TopBar.vue";
	export default {
		data() {
			return {
				navIndex:0,
				nav:[{
					ref: 'product',
					title: '商品'
				},{
					ref: 'comment',
					title: '评价'
				},{
					ref: 'detail',
					title: '详情'
				},{
					ref: 'recommend',
					title: '推荐'
				}]
			}
		},
		methods:{
			scroll(ref){
				console.log(this.$refs[ref]);
				var top=this.$refs[ref].offsetTop;
				document.documentElement.scrollTop=top;
			}
		},
		mounted() {
			
			console.log(document.body.scrollTop);
			
		},
		components: {
			TopBar,
		}
	}
</script>

<style scoped="scoped">
	.top-bar{
		width: 100%;
		position: fixed;
		top:0px;
		left: 0px;
	}
	.top-bar ul{
		margin: 0px;
		padding: 0px 40px;
		text-align: center;
		list-style: none;
	}
	.top-bar ul li{
		float: left;
		width: 25%;
	}
	.top-bar ul li.active span{
		font-weight: bold;
		color: #e4393c;
		padding-left: 13px;
		background: url(data:image/png;base64,) no-repeat 0;
		background-size: 8px;
	}
</style>